...
def test_jcs_orders_keys_by_utf16_code_units():
    assert canonicalize_jcs({"\ue000": 1, "\U00010000": 2}).decode() == (
        '{"𐀀":2,"\ue000":1}'
    )


def test_jcs_rejects_floats_and_unsafe_integers():
...
def test_action_id_matches_published_conformance_vector():
    assert canonical_action_id(envelope()) == (
        "sha256:5efc8759c0a4fb5ab9b33a1a0d8b9ca"
        "69d123eaac8d6c643e7a271906ce1b11d"
    )


def test_action_id_is_stable_and_rejects_unbound_fields():
    left = envelope()
...
def test_authorization_deadline_is_bound_when_present():
...
def test_continuation_fails_closed_for_malformed_envelope():
    frozen = canonical_action_id(envelope())
    assert continuation_matches(frozen, {**envelope(), "extra": "drift"}) is False
    assert continuation_matches("bad", envelope()) is False


def test_decision_id_normalizes_semantic_sets():
    ...
    assert canonical_decision_id(action_id, left) == canonical_decision_id(
        action_id, right
    )


def test_decision_id_changes_with_authority_or_evidence():
    action_id = canonical_action_id(envelope())
    baseline = canonical_decision_id(action_id, decision())
    assert baseline != canonical_decision_id(
        action_id, decision(permitted_next_transition="AUDIT_ONLY")
    )
    assert baseline != canonical_decision_id(
        action_id, decision(evidence_refs=["sha256:" + "d" * 64])
    )


def test_chain_record_binds_upstream_and_payload():
    ...
    assert first != canonical_chain_record_id("audit_record", upstream, payload)
    assert first != canonical_chain_record_id(
        "execution_outcome", sha256_jcs({"decision": "deny"}), payload
    )
